...
```{r}
norm_echo_df <- data.frame(sentence_no = 1:(max_sent_freq*length(sentence_ids)), full_norm_echoes)
norm_echo_sliver <- norm_echo_df %>% pivot_longer(-sentence_no, names_to = "column", names_prefix = "X", names_transform = list(column = as.numeric), values_to = "echo_value") %>% filter(sentence_no %in% 1:2) %>% filter(column %in% 110:130)
ggplot(norm_echo_sliver, aes(x = column, y = echo_value, group = as.factor(sentence_no), color = as.factor(sentence_no))) + geom_line()
```
